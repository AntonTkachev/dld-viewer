...
Pipeline:
  ...
  _data_communities.geojson     (OSM polygons, used as geometry source for sub-zones)
            │
            ▼
...
For each split entry in the override file:
  ...
  2. Add one new feature per sub-polygon, with geometry copied from
     `_data_communities.geojson[osm_polygon]` and a filter pinning it to
...
OSM_SRC     = os.path.join(ROOT, '_data_communities.geojson')
