...
// Read ?q=… on load and pre-fill the search box. Used by deep-links from
// the map's construction-layer popup ("Open all 18 →" on Motor City marker
// lands here with ?q=Motor+City already applied). Also clears the default
// status filter so the user sees the whole pipeline, not just in-flight.
function applyUrlParams() {{
  const params = new URLSearchParams(window.location.search);
  const q = params.get("q");
  if (q) {{
    STATE.search = q;
    // Drop the default in-flight filter so finished + active are all shown
    // — the user came in via a specific district lookup, not a survey of
